...
function joomlabridge_config() 
{
	return array(
		...
		"description" 	=> "WHMCS Joomla Bridge will allow you to use Joomla, the award-winning content management system as your primary promotion site, leaving WHMCS for the billing & client management. Our bridge will take over Joomla's user database, any user registered through WHMCS will be synched to Joomla, including login shares !",
		"version" 	=> "1.0.5",
...
<ul class="nav nav-tabs admin-tabs">
	...
	<?php $selected = ($page_name == $page || ($page_name == 'modules' && $page == 'modulesmanage')) ? true : false; ?>
	<li<?php if($selected) { ?> class="active"<?php } ?>><a href="<?php echo $modulelink; ?>&page=<?php echo $page_name; ?>"><?php echo $vars['_lang']['page_' . strtolower($page_name)]; ?></a></li>
	<?php } ?>
</ul>
			...
			switch($action)
			{
				...
				case 'save':
					...
					if(sizeof($rule_find) && is_array($rule_find))
					{
						...
						{
							$rule_find_data = mysql_real_escape_string($rule_find_data);
							$rule_replace_data = mysql_real_escape_string($_POST['rule_replace'][$rule_id]);
					...
					// add new rule
					$new_rule_find = mysql_real_escape_string($_POST['new_rule_find']);
					$new_rule_replace = mysql_real_escape_string($_POST['new_rule_replace']);
...
$(document).ready(function() {
	...
	$('#testhtml').click(function() {
		...
		shadow.fadeIn('slow');
		<?php $getModulesData = $joomlaBridge->getModulesData($module_id, true); ?>
		contentBox.html('<pre><code><?php echo mysql_real_escape_string(str_replace("\n", "<br />", htmlentities($getModulesData['success'] ? $getModulesData['data'] : ''))); ?></code></pre>').fadeIn('slow');
...
<h4 style="margin: 20px 0 5px;">Editing Module "<?php echo $module_details['module_name']; ?>"</h4>
...
<h4 style="margin: 20px 0 5px;">
	Manage HTML Attributes Rules
	<input type="button" class="btn btn-primary" name="submit" id="testhtml" onclick="return false;" value="Test HTML" />
</h4>
...
<div class="tablebg">
	...
	With Selected:
	<input type="submit" class="btn btn-danger" name="deletemulti" value="Delete" /> 
</div>

<p align="center">
	<input type="submit" class="btn btn-primary" name="save" value="<?php echo $vars['_lang']['save_changes']; ?>" />
	<input type="button" class="btn btn-default" name="submit" onclick="window.location='<?php echo $modulelink; ?>&page=modules';" value="Back to Modules List" />
</p>
...
<form action="<?php echo $modulelink; ?>&page=modules" method="post">
	<div class="tablebg">
		...
		<?php if($modulelist['success'] && sizeof($modulelist['data'])) { ?>
		<?php foreach($modulelist['data'] as $module_data) { ?>
		<tr>
		...
		case 'settings':
			...
			switch($action)
			{
				...
				case 'clearcache':
					...
					$clearCache = $joomlaBridge->clearCache();
...
<h4 style="margin: 20px 0 5px;">Joomla Login Details</h4>
<table width="100%" cellspacing="2" cellpadding="3" border="0" class="form">
<tbody>
<tr>
...
<h4 style="margin: 20px 0 5px;">Users Synchronization</h4>
<table width="100%" cellspacing="2" cellpadding="3" border="0" class="form">
<tbody>
<tr>
...
<tr>
	...
	<td class="fieldarea">
		<select name="config[username_format]" class="form-control select-inline">
...
<h4 style="margin: 20px 0 5px;">Modules Managment</h4>
<table width="100%" cellspacing="2" cellpadding="3" border="0" class="form">
<tbody>
<tr>
...
<tr>
	...
	<td class="fieldarea">
		...
		<input type="submit" class="btn btn-success clear-cache" onclick="window.location='<?php echo $modulelink; ?>&page=settings&action=clearcache'; return false;" value="Clear Now" />
...
<p align="center">
	...
	<input type="submit" class="btn btn-primary" name="submit" value="<?php echo $vars['_lang']['save_changes']; ?>" />
</p>
